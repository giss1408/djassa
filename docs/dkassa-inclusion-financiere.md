# Dkassa — Custom Instruction : Volet Inclusion Financière
...
### 2bis. Proposer l'épargne — Dkassa intermédiaire, jamais dépositaire
- Principe non négociable : **l'argent épargné ne transite jamais par un compte détenu par Dkassa.** Chaque versement va directement du portefeuille mobile money de l'utilisateur vers un compte détenu par un partenaire agréé (IMF, banque, ou établissement de monnaie électronique). Dkassa orchestre et affiche, il ne détient pas — exactement le même principe que pour le crédit (Dkassa ne prête pas).
- Cette contrainte n'est pas seulement réglementaire (voir section contraintes ci-dessous), elle doit aussi être visible pour l'utilisateur : l'app doit toujours indiquer clairement quel partenaire détient effectivement les fonds, pour construire la confiance plutôt que la faire reposer sur une opacité.
- Deux formats d'épargne à prioriser, dans cet ordre :
  1. **Épargne bloquée à objectif** (ex. : épargner pour du stock, un équipement, une période de soudure) — retrait bloqué jusqu'à la date ou au montant cible, hébergé chez le partenaire agréé.
  2. **Épargne liée au score de fiabilité** — un historique d'épargne régulière alimente le même score interne que la fidélité et la tontine (voir point 3), renforçant la preuve de solvabilité présentée à un partenaire de crédit.
- Ne pas construire un produit d'épargne concurrent des IMF partenaires : Dkassa est le canal de collecte et de suivi, l'IMF reste le dépositaire et le gestionnaire réglementé des fonds.

### 3. Générer un score de fiabilité interne, avant tout score de crédit formel
- Construire un score simple et explicable (régularité des achats, respect des cycles de tontine, régularité d'épargne, ancienneté) — pas une boîte noire.
...
- **Ne pas détenir de fonds d'épargne, même temporairement.** Aucun versement d'épargne ne doit transiter par un compte propre à Dkassa, y compris pour un simple "passage" technique — le flux doit aller directement du client vers le compte du partenaire agréé. Un compte de passage chez Dkassa équivaudrait à de la collecte de dépôts, ce qui nécessite un agrément que Dkassa ne détient pas.
...
4. Pour tout produit d'épargne, vérifier avant lancement que le flux d'argent va bien directement du client au compte du partenaire agréé, sans étape intermédiaire chez Dkassa — un audit technique du flux de fonds doit précéder tout lancement, pas le suivre.
...
3bis. **Commission d'apport sur les produits d'épargne** — l'IMF ou l'établissement partenaire qui héberge les fonds verse une commission à Dkassa pour chaque compte d'épargne ouvert et alimenté via l'app (logique identique à la commission de crédit, appliquée à la collecte d'épargne plutôt qu'au prêt). Dkassa ne touche jamais aux fonds eux-mêmes, seulement à la commission de distribution.
...
| **2bis — Épargne intermédiée** | Produit d'épargne bloquée à objectif, flux direct client → partenaire agréé, aucune détention de fonds par Dkassa | Score formel intégrant l'épargne, commission d'apport |
...
- Volume total épargné via l'app et hébergé chez les partenaires agréés, et % d'objectifs d'épargne atteints sans retrait anticipé
