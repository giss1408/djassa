...
from ..schemas import Transaction, TransactionOut, Merchant
...
async def create_transaction(payload: Transaction, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    return TransactionOut(id=txn.id, merchant_id=txn.merchant_id, user_id=txn.user_id, amount=float(txn.amount), currency=txn.currency, type=txn.type, timestamp=txn.timestamp)
...
async def list_transactions_for_merchant(merchant_id: int, limit: int = Query(50, ge=1, le=100), db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    q = select(TransactionModel).where(TransactionModel.merchant_id == merchant_id).order_by(TransactionModel.timestamp.desc()).limit(limit)
    res = await db.execute(q)
    txns = res.scalars().all()
    return [TransactionOut(id=t.id, merchant_id=t.merchant_id, user_id=t.user_id, amount=float(t.amount), currency=t.currency, type=t.type, timestamp=t.timestamp) for t in txns]
